...
Each day is a flat task list. Whoever has the capacity picks the next unblocked item and ships it. No ownership column — coordinate in chat.
...
- [ ] Fork `google-ai-edge/gallery` to a personal GitHub account
- [ ] Clone the fork in Android Studio at `D:\Projects\hackathon\gallery\Android\src`
- [ ] First Gradle sync (~15-30 min — leave it running)
- [ ] Connect a test phone via USB; enable USB debugging
- [ ] Run the unmodified gallery on phone; confirm an LLM tile loads
- [ ] Paste the contents of `synthesis_cache_scenario_a.txt` (Colab) back to chat so the cached scenario can be identified and integrated
- [ ] `git pull` `nusasiaga` main to get all the unified-picker work
- [ ] Run `npm install && npm run dev` locally on the dashboard to confirm the merged tree builds
- [ ] Deploy NusaSiaga to Vercel for the Live Demo URL (free tier is fine)
- [ ] Record the Vercel URL in the README so we have one canonical Live Demo link
- [ ] Set `NASA_FIRMS_MAP_KEY` in the Vercel project settings

### Day 3 — 15 May

- [ ] Drop the four `ai.grg.*` Kotlin files into the gallery fork at the right package path
...
- [ ] Render the resulting `EdgeTriageReport` as a result card with severity, hazards, immediate action, and routing badge
- [ ] Confirm Gemma 4 E2B downloads and runs inside the customized app
- [ ] Update Hazard Analyzer copy on the dashboard wildfire view: "Gemma 3n" → "Gemma 4"
- [ ] Capture screenshots of both Wildfire and Flood views from the Vercel deployment for the video editor
- [ ] Draft the video storyboard in `writeup/video_script_v1.md` (refine the existing one with the unified-picker visuals)
...
- [ ] Take fresh dashboard screenshots showing all three disaster types with full operational dashboards
- [ ] Begin filming phone footage — responder snapping a photo, triage appearing on the result card
...
- [ ] Film the 3-minute video. Scenes:
  ...
  5. Architecture beat (E2B → 31B → dashboard)
- [ ] Edit, voice-over, music. Upload to YouTube unlisted; share link in writeup
- [ ] Optional: Unsloth LoRA fine-tune of E2B on curated disaster images
- [ ] Optional: add a wildfire user-report scenario (Riau peatland fire, ~10 hand-crafted reports)
...
- [ ] Dry run: open the Kaggle writeup form, paste everything, preview, then submit
- [ ] **Submit at least 6 hours before deadline (11:59 PM UTC, Monday 18 May).** That is 06:59 AM Tuesday 19 May Jakarta time (WIB).
...
6. **Optional: APK file** attachment — built on Day 5 once the Android UI works
...
- **One person on the dashboard at a time** for big component refactors. Cosmetic polish in parallel is fine.
...
| Vercel deploy fails on FIRMS API key | Set the env var; if still broken, the wildfire view falls back to notebook-JSON automatically |
| Video doesn't finish on Day 5 | Cut to a 90-second version focused on the Flood synthesis dashboard + phone airplane-mode shot |
...
- **Live demo URL:** (added Day 2 after the Vercel deploy)
