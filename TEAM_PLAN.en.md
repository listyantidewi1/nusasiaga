...
*The set-up-everything-physical day. Android Studio is downloading a 5+ GB SDK; the Vercel deploy needs an env var; the test phone needs USB debugging enabled. None of this is intellectually hard, but all of it must happen before Days 3-5.*
...
- [ ] First Gradle sync (~15-30 min — leave it running, don't cancel)
- [ ] Connect a test phone via USB; enable USB debugging from Developer Options
- [ ] Run the unmodified gallery on phone; confirm an LLM tile loads a model (any model — we just need to know inference works)
- [ ] Paste the contents of `synthesis_cache_scenario_a.txt` (Colab cache) into chat so we can identify which scenario it contains and integrate that synthesis JSON into the dashboard
- [ ] `git pull` `nusasiaga` main to get all the unified-picker and scenario work
...
|---|---|---|
| **Android Gemma 4 E2B download fails** because the gallery requires HuggingFace OAuth and we haven't set up developer app credentials | Medium | First, try the gallery's default model picker without OAuth — many models are public. If only E2B requires OAuth, set up the HF developer app per the gallery's `DEVELOPMENT.md` and update `ProjectConfig.kt`. Worst case: use a smaller pre-bundled model in the demo, write the writeup as "architecture proven; model bundle to follow post-hackathon" |
